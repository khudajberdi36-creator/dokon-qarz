          ...
          >
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 18px', borderBottom: '1px solid var(--border)' }}>
              ...
              <input
                ...
                placeholder="Qarzdor yoki mahsulot qidiring..."
                style={{
                  ...
                  {results.qarzdorlar.length > 0 && (
                    <div>
                      <div style={{ padding: '10px 18px 4px', fontSize: 11, fontWeight: 700, color: 'var(--text3)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>
                        🧾 Qarzdorlar ({results.qarzdorlar.length})
                      </div>
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      autoTable(doc, {
        ...
        body: [
          ["Jami qarzdorlar", `${stats.jami_qarzdorlar} kishi`],
      ...
      for (let i = 1; i <= pageCount; i++) {
        ...
        doc.text(
          `Do'kon Qarz — ${davr} hisobot | Sahifa ${i} / ${pageCount}`,
          pageW / 2, doc.internal.pageSize.getHeight() - 8,
          { align: 'center' }
        );
      }

      const fileName = `dokon-qarz-hisobot-${tur === 'oylik' ? `${yil}-${String(oy).padStart(2,'0')}` : yil}.pdf`;
      doc.save(fileName);
    } catch (err) {
          ...
          {previewLoading ? (
            ...
            <div style={{ background: 'var(--bg3)', borderRadius: 12, padding: '16px', border: '1px solid var(--border)' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                {[
                  { icon: '🧾', label: 'Jami qarzdorlar', val: `${preview.stats.jami_qarzdorlar} ta` },
              ...
              <div style={{ marginTop: 12, fontSize: 11, color: 'var(--text3)', borderTop: '1px solid var(--border)', paddingTop: 10 }}>
                📋 Hisobotda: Umumiy statistika • Oylik dinamika • Top qarzdorlar • Mahsulotlar holati
              </div>
            </div>
          )}
        </div>
