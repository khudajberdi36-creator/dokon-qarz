...
// ✅ YANGI: Admin secret bilan parolni qayta o'rnatish
// Ishlatish: POST /api/auth/reset-password
// Body: { secret: "RESET_SECRET dan env", username: "begzod", new_password: "yangiparol" }
router.post('/reset-password', async (req, res) => {
  ...
  if (!username || !new_password)
    return res.status(400).json({ error: "username va new_password kerak" });
...
// Parolni o'zgartirish (foydalanuvchi o'zi uchun)
router.post('/change-password', async (req, res) => {
  ...
  try {
    ...
    if (!old_password || !new_password)
      return res.status(400).json({ error: 'Eski va yangi parol kiritilmadi' });
...
router.post('/register', (req, res) => {
  res.status(404).json({ error: "Bu funksiya mavjud emas" });
});
